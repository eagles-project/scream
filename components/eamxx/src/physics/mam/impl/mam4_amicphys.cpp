...
KOKKOS_INLINE_FUNCTION int numptr_amode_gas_pcnst(const int mode) {
  static constexpr int numptr_amode_gas_pcnst_[num_modes] = {
      13, 18, 26, 30};
...
KOKKOS_INLINE_FUNCTION int lmap_aer(const int iaer, const int mode) {
  static constexpr int
      lmap_aer_[AeroConfig::num_aerosol_ids()][num_modes] = {
...
void construct_subareas_1gridcell(
    ...
    const Real q_pregaschem[gas_pcnst],     // in qq TMRs before
                                            // gas-phase chemistry
    const Real q_precldchem[gas_pcnst],     // in qq TMRs before
                                            // cloud chemistry
    const Real qqcw_precldchem[gas_pcnst],  // in  qqcw TMRs before
                                            // cloud chemistry
    const Real qq[gas_pcnst],    // in current tracer mixing ratios (TMRs)
                                 // *** MUST BE  #/kmol-air for number
                                 // *** MUST BE mol/mol-air for mass
    const Real qqcw[gas_pcnst],  // in like qq but for
    ...
    Real qaerwatsub3[num_modes][maxsubarea()]  //,  // out aerosol water mixing
                                               // ratios (mol/mol)
    // Real qaerwat[num_modes]  // in  aerosol water mixing ratio
    //  (kg/kg, NOT mol/mol)
) {
...
void mam_amicphys_1subarea_clear(
    const AmicPhysConfig &config, const Real deltat, const int jsub,
    ...
    const Real pblh, const Real relhum, Real dgn_a[num_modes],
    Real dgn_awet[num_modes], Real wetdens[num_modes],
    ...
    const Real qnum3[num_modes], Real qnum4[num_modes],
    Real qnum_delaa[num_modes][nqtendaa()],
    const Real qaer3[AeroConfig::num_aerosol_ids()][num_modes],
    Real qaer4[AeroConfig::num_aerosol_ids()][num_modes],
    Real qaer_delaa[AeroConfig::num_aerosol_ids()][num_modes][nqtendaa()],
    const Real qwtr3[num_modes], Real qwtr4[num_modes]) {
...
void mam_amicphys_1subarea_cloudy(
    ...
    const Real qnum3[num_modes], Real qnum4[num_modes],
    Real qnum_delaa[num_modes][nqtendaa()],
    const Real qaer2[AeroConfig::num_aerosol_ids()][num_modes],
    const Real qaer3[AeroConfig::num_aerosol_ids()][num_modes],
    Real qaer4[AeroConfig::num_aerosol_ids()][num_modes],
    Real qaer_delaa[AeroConfig::num_aerosol_ids()][num_modes][nqtendaa()],
    const Real qwtr3[num_modes], Real qwtr4[num_modes],
    const Real qnumcw3[num_modes], Real qnumcw4[num_modes],
    Real qnumcw_delaa[num_modes][nqqcwtendaa()],
    const Real qaercw2[AeroConfig::num_gas_ids()][num_modes],
    const Real qaercw3[AeroConfig::num_gas_ids()][num_modes],
    Real qaercw4[AeroConfig::num_gas_ids()][num_modes],
    Real qaercw_delaa[AeroConfig::num_gas_ids()][num_modes][nqqcwtendaa()]) {
  //
  // calculates changes to gas and aerosol sub-area TMRs (tracer mixing ratios)
...
void mam_amicphys_1gridcell(
    ...
    const Real relhumsub[maxsubarea()], Real dgn_a[num_modes],
    Real dgn_awet[num_modes], Real wetdens[num_modes],
  ...
  for(int jsub = 0; jsub < nsubarea; ++jsub) {
    ...
    if(iscldy_subarea[jsub]) {
      mam_amicphys_1subarea_cloudy(
          sub_config, deltat, jsub, nsubarea, iscldy_subarea[jsub],
...
KOKKOS_INLINE_FUNCTION
void set_subarea_qnumb_for_intrst_aerosols(const int kk,
    ...
    Real (&qsubx)[gas_pcnst][maxsubarea()])               // inout
{
  ...
  for(int imode = 0; imode < num_modes; ++imode) {
    ...
    if(kk == 48 ) printf("set_subarea_qnumb_for_intrst_aerosols:   %0.15E, %0.15E, %0.15E, %0.15E, %0.15E, %0.15E, %0.15E, %0.15E, %0.15E, %i\n",qsubx[icnst][jclea], qsubx[icnst][jcldy],qgcmx[icnst],factor_clea, factor_cldy, qgcm_intrst, qgcm_cldbrn, fcldy, fclea, icnst); 
...
// FIXME: Remove unused variables in the arg list!
KOKKOS_INLINE_FUNCTION
...
if(kk == 48) {
 ...
 /////
  ...
  mam_amicphys_1gridcell(config, deltat, nsubarea, ncldy_subarea,

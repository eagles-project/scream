    ...
    Real (&qsub_b)[gas_pcnst()][maxsubarea()])  // inout
{
  ...
  for(int icnst = 0; icnst < gas_pcnst(); ++icnst) {
    if(lcompute[icnst]) {
      ...
      if(kk == 48)
        // printf("HERE3:   %0.15E,   %0.15E,   %0.15E,   %0.15E,   %0.15E, %i
        // %i %i\n",
...
void mam_amicphys_1gridcell(
    // in
    const int kk, const AmicPhysConfig &config, const int nstep,
      ...
      for(int imode = 0; imode < num_modes; ++imode) {
        ...
        for(int iaer = 0; iaer < num_aerosol_ids; ++iaer) {
          ...
          if(la > 0) {
            ...
            // if(kk == 48)
            //   printf("qaercw3:   %0.15E,   %0.15E,   %0.15E, %i  %i\n",
            //   qaercw3[iaer][imode],
    ...
    for(int jsub = 1; jsub <= nsubarea; ++jsub) {
      for(int iq = 0; iq < nqqcwtendaa(); ++iq) {
        for(int icnst = 0; icnst < gas_pcnst(); ++icnst) {
          ...
          //          qqcwgcm_tendaa[icnst][iq],
          //          qqcwsub_tendaa[icnst][iq][jsub], icnst, iq, jsub);
        }
      }
    }
  }  // if (ncldy_subarea
...
void modal_aero_amicphys_intr(
  ...
  */
  ...
  if(kk == 48) {
    // printf("QSAT:   %0.15E,   %0.15E,   %0.15E,   %0.15E,   %0.15E,
    // %0.15E\n", qv_sat, qv, pmid, temp,
    //        epsqs, ev_sat);
  }
  ...
  if(kk == 48) {
    for(int icnst = 0; icnst < 6; ++icnst) {
      // printf("amic2a:   %0.15E,   %0.15E,   %0.15E,  %i\n", qgcm1[icnst],
      // qgcm2[icnst],
      //        qgcm3[icnst], icnst);
    }
  }
  ...
  set_subarea_gases_and_aerosols(kk, nsubarea, jclea, jcldy, fclea,
                                 ...
                                 qqcwsub3);                      // out
  if(kk == 48) {
    // printf("amic1:%i %i %i %i,%s,%s,   %0.15E,   %0.15E,   %0.15E, %0.15E\n",
    // nsubarea,
  ...
  mam_amicphys_1gridcell(
      // in
      kk, config, nstep, deltat, nsubarea, ncldy_subarea, iscldy_subarea,
      afracsub, temp, pmid, pdel, zm, pblh, relhumsub, dgn_a, dgn_awet, wetdens,
      qsub1, qsub2, qqcwsub2, qsub3, qqcwsub3,
      // inout

...
void MAMWetscav::run_impl (const double dt)
{
  ...
  /*Fortran code:
  call modal_aero_calcsize_sub(state%ncol, state%lchnk, state%q, state%pdel, dt, & !in
     qqcw, ptend, dgnumdry_m=dgncur_a) !inout
   ...
   //mam4::CalcSizeProcess process_(aero_config_); //initiate MAM4xx calcsize process
   ...
   /* 
    * -- NOTES: 1. Flags for the inter-mode particle transfer (do_aitacc_transfer) and  size adjustment (do_adjust) are TRUE by default
    *                a. Size adjustment is only done by changing aerosol numbers in the modes.
    *           2. Interstitial and cld borne aerosols (i.e. "tends") mmr will be updated (update_mmr is TRUE by default) 
    */
   
   // -- call the process to compute size
   static constexpr Real t = std::numeric_limits<Real>::max(); // This var is never used, so setting it to an unrealistic value
...
/*
    ...
    mmode_loop_aa: do mtmp = 1, ntot_amode ! main loop over aerosol modes
       ...
       lphase_loop_aa:  do lphase = 2,1,-1  ! do cloudborne (2) first then interstitial (1)
          ...
          lspec_loop_aa: do lspec = 1, nspec_amode(imode)+2 ! loop over number + chem constituents + water
             ...
             if (mm <= 0 .or. jnummaswtr == jaerowater ) cycle  ! by pass wet aerosols

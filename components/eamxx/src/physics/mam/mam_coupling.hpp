...
void state_q_for_column_at_one_lev(const AerosolState &dry_aero,
  ...
  if(dry_aero.gas_mmr[0].data()) { // if gases are defined in dry_aero aerosol state
    s_idx = gasses_start_ind();  // gases start at index 9 (index 10 in Fortran version)
    for(int g = 0; g < num_aero_gases(); ++g) {
      EKAT_KERNEL_ASSERT_MSG(dry_aero.gas_mmr[g].data(),
                             "gas_mmr not defined for dry aerosol state!");
      // get mmr at level "klev"
      state_q(s_idx) = ekat::subview(dry_aero.gas_mmr[g], column_index)(klev);
      s_idx++; //update index
    }
  }
  else{
  ...
  for(int m = 0; m < num_aero_modes(); ++m) {
    ...
    for(int a = 0; a < num_aero_species(); ++a) {
      if(dry_aero.int_aero_mmr[m][a].data()) {
        state_q(s_idx) =
            ekat::subview(dry_aero.int_aero_mmr[m][a], column_index)(klev);
